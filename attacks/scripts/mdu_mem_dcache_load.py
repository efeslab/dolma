...
# Modifies the assembly output of compilation for mdu_mem_dcache_load
...
with open(parent_dir / '../bin/mdu_mem_dcache_load_pre_mod.s', 'r') as asm: 
    data = asm.read().replace(''.join(access_phase), ''.join(access_phase_modified))
        
asm.close()

with open(parent_dir / '../bin/mdu_mem_dcache_load_post_mod.s', 'w') as asm:
    asm.write(data)
asm.close()